				...
				<table class="widefat striped health-check-table">
					...
					foreach ( $details['fields'] as $field_name => $field ) {
						...
						if ( in_array( $field_name, $sizes_fields, true ) ) {
							printf( '<tr><th scope="row">%s</td><td class="%s">%s</td></tr>', esc_html( $field['label'] ), esc_attr( $field_name ), $values );
						} else {
							printf( '<tr><th scope="row">%s</td><td>%s</td></tr>', esc_html( $field['label'] ), $values );
						}
					}
